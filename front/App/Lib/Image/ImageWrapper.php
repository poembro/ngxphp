...
class ImageWrapper
{
	...
	*/
	...
	/**
	* @desc 暂存的验证码session id
	* @var string
	* @access private
	*/
	private static $_sessionId = 'Tiny_Verify';
	...
	*/
	...
	/**
	* @desc  生成缩略图  缩略图会根据源图的比例进行缩略的，生成的缩略图格式是JPG
	* @access public
	...
	* @return bool
	*/
	public static function getThumbnail($srcfile, $dstfile, $maxThumbWidth, $maxThumbHeight, $rate = null, $type = self::IMAGE_TYPE_STREAM)
	{
		$tow = $maxThumbWidth;
		$toh = $maxThumbHeight;
		$makeMax = 0;
	...
	*/
	public static function imgVerify($length = 6, $mode = 3, $type = 'png', $width = 80, $height = 30)
	{
		...
		$_SESSION[self::$_sessionId] = md5(strtolower($randval));
	...
	*/
	...
	{
		$res = $_SESSION[self::$_sessionId] == md5(strtolower($verifyCode));
		self::cleanVerify();
		return $res;
	}
	...
	*/
	...
	{
		unset($_SESSION[self::$_sessionId]);
	}

	/**
	* @desc 获取一些随机字符串 主要为生成验证码
	* @access public
